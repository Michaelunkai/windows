@echo off
chcp 437 >nul
title WINDOWS 11 STARTUP FIX
color 0E

echo.
echo    ========================================
echo       WINDOWS 11 STARTUP FIX
echo    ========================================
echo.
echo    Fix startup issues:
echo    * Slow boot times
echo    * Too many startup programs
echo    * Startup delays
echo.
echo    Launching as Administrator...
echo.

set "SCRIPT=%~dp0StartupFix.ps1"
if not exist "%SCRIPT%" (
    echo ERROR: Script not found: %SCRIPT%
    pause
    exit /b 1
)

powershell -ExecutionPolicy Bypass -Command "Start-Process powershell -ArgumentList '-ExecutionPolicy Bypass -NoProfile -File \"%SCRIPT%\"' -Verb RunAs -Wait"

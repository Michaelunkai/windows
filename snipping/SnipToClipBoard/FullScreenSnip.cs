...
class FullScreenSnip : Form
{
    ...
    {
        // FIX: Delay allows hotkey modifier keys (Alt, Ctrl) to be physically released
        // before the overlay appears. Without this delay, Alt is still held when the
        // overlay first receives a click, causing WM_SYSCOMMAND SC_KEYMENU or SC_MOVE
        // to fire and destroy the overlay on the first mouse press.
        Thread.Sleep(150);
        Application.DoEvents(); // Flush any pending key-up messages

        SnipOverlay overlay = new SnipOverlay(this, forceImageClipboard);
        overlay.Show();
        // FIX: Force the overlay to the foreground so it receives keyboard and mouse focus
        overlay.Activate();
    }
...
class SnipOverlay : Form
{
    ...
    // WM_SYSCOMMAND — intercepted to block system-level window commands
    private const int WM_SYSCOMMAND = 0x0112;
    // System command codes that must be blocked to prevent Alt+click from destroying the overlay
    private const int SC_MOVE      = 0xF010; // Triggered by Alt+drag — moves window
    private const int SC_SIZE      = 0xF000; // Triggered by Alt+border — resizes window
    private const int SC_MINIMIZE  = 0xF020;
    private const int SC_MAXIMIZE  = 0xF030;
    private const int SC_CLOSE     = 0xF060;
    private const int SC_KEYMENU   = 0xF100; // Triggered by Alt alone or Alt+key — opens system menu
    private const int SC_MOUSEMENU = 0xF090; // Right-click on title bar — opens system menu
    ...
    public SnipOverlay(FullScreenSnip parent, bool forceImageClipboard)
    {
        ...
        // FIX: Set painting styles before the window handle is created for flicker-free rendering
        this.SetStyle(
        ...
        this.Opacity = 1.0;
        this.KeyPreview = true; // FIX: Ensure KeyDown fires even if a child has focus

        // Capture the screen BEFORE the overlay window appears so the screenshot is clean
    ...
    private void OnMouseUp(object sender, MouseEventArgs e)
    {
        ...
        {
            // FIX: Always release mouse capture on mouse-up regardless of outcome
            this.Capture = false;
            isSelecting = false;
